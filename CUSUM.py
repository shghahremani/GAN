...
data = pd.read_csv('Gussian_model_dataset/modelica_testset.csv')
columnsName_train=['P','PE','T','attack']
data.columns=columnsName_train
pressure=data[['P']]
power=data[['PE']]
temperature=data[['T']]
print(power[:300])
ta, tai, taf, amp = detect_cusum(pressure[300:601], 0, .02, True, True)
ta, tai, taf, amp = detect_cusum(power[300:601], 2, 0.2, True, True)
ta, tai, taf, amp = detect_cusum(temperature[300:601], 2, .02, True, True)
